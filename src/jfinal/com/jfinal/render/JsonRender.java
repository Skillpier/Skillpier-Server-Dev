...
public class JsonRender extends Render {
	...
	public JsonRender(final String key, final Object value) {
		...
		this.jsonText = JsonKit.mapToJson(new HashMap<String, Object>() {
			{
				put(key, value);
			}
		}, convertDepth);
	...
	public JsonRender(String jsonText) {
		if (jsonText == null)
			throw new IllegalArgumentException(
					"The parameter jsonString can not be null.");
...
//		String tmpJson = jsonText;
//		//render with ' by kingsley
//		int index = -1;
//		while ((index = tmpJson.indexOf("\"")) != -1) {
//			int tmp = index - 1;
//			char before = tmpJson.charAt(tmp);
//			if (before != '\\') {
//				String one = tmpJson.substring(0, index);
//				String two = tmpJson.substring(index + 1);
//				tmpJson = one + "\'" + two;
//			}
//		}
		System.out.println(jsonText);
		jsonText = JsonFormater.format(jsonText);
