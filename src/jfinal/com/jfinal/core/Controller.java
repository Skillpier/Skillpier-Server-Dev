...
public abstract class Controller {
	...
	public UploadFile getFile(String parameterName, String saveDirectory) {
		...
		if (parameterName.contains(",")) {
			parameterName = parameterName.split(",")[0].split("=")[1];
		}
		// end
	 ...
	 *         Render with api
	 */
	public void renderApi(String attr) {
		String source = getPara("request_source", null);
		if (source == null) {
			render = renderFactory.getJsonRender();
		} else {
			setAttr("response", JsonKit.toJson(getAttr(attr)));
			int index = getParaToInt("request_index");
			ApiDescription apiBean = BuildAPI.getApis().get(index);
			setAttr("r", apiBean);
			render = renderFactory.getFreeMarkerRender("/admin/FormResponse.html");
		}
	}
	 ...
	 * Render with attributes set by setAttr(...) before.
	 * <p>
	 * Example: renderJson("blogList", "user");
	 * 
	 * @param quote
	 *            :1=",0='
	 * @param attr
	 *            :为多个setAttr的键值
	 */
	public void renderMultiJson(String... attr) {
		System.out.println("get attr =" + getAttr(attr[0]).toString());
		String type = getPara("invoke", "app");
		if (type.equals("h5")) {
			this.renderJson((String)getAttr(attr[0]).toString());
		} else {
			String attrs = "";
			for (String string : attr) {
				attrs = attrs + string + ",";
			}
			render = renderFactory.getJsonRender(attrs.split(","));
		}
	}
