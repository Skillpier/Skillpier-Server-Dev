...
public class CouponManage extends Controller {
	...
	public void list() {
		...
		Page<Coupon> coupon = Coupon.dao.paginate(CurrentPage, PAGE_SIZE,
				"select * ", "from coupon where status=1 and coupon_type=1 "
						+ my_sql + "order by coupon_number desc,post_time desc");
		for (Coupon coupons : coupon.getList()) {
			...
			String nickname = "";
			if (is_seller!=0) {
				User users = User.dao.findById(is_seller);
				if (users != null) {
					nickname = users.getStr(users.nickname);
				}
			}
			coupons.put("nickname", nickname);
			int is_course = coupons.get(coupons.is_course);
			String title = "";
			if (is_course!=0) {
				Course courses = Course.dao.findById(is_course);
				if (courses != null) {
					title = courses.getStr(courses.title);
				}
			}
			coupons.put("course_title", title);
		}
		if (message1 != null) {
		...
		render("/admin/CouponManageList.html");
	}
	...
	public void addCoupon() {
		...
		render("/admin/CouponManageAdd.html");
	}
	// 添加优惠劵
	public void addCoupons() {
		...
		boolean save = coupon.set(coupon.provider, provider)
				...
				.save();
		if (save) {
			redirect("/admin/CouponManage/list?message1=1");
		} else {
			redirect("/admin/CouponManage/list?message1=2");
		}
	}
	...
	public void delete() {
		...
		redirect("/admin/CouponManage/list");
	}

	public void addEditor() {
		int coupon_id = getParaToInt("coupon_id");
		// 商家指定
		List<User> users = User.dao.find("select user_id,nickname from user where status=1 and agent_level=2");
		setAttr("users", users);
		// 一级分类
		List<Category01> categorys01 = Category01.dao.find("select category_01_id,category_01_name from category_01 where status=1");
		setAttr("categorys01", categorys01);
		Coupon coupon = Coupon.dao.findById(coupon_id);
		setAttr("coupon", coupon);
		if(coupon!=null){
			int cateid=coupon.get(coupon.category_01_id);
			List<Category02> categorys02 = Category02.dao.find("select category_02_id,category_01_id,category_01_name,category_02_name from category_02 where category_01_id =? and status=1",cateid);
		...
		render("/admin/CouponManageEditor.html");
	}
   //根据一级分类查询二级分类
	public void selectCategory_02(){
		...
		if (sid != 0){
			 lists = Category02.dao.find("select category_02_id,category_02_name from category_02 where category_01_id=? and status=1",sid);
		}
	...
	public void addCouponsEditor() {
		...
		boolean update = coupon.set("provider", provider)
				...
				.set("usable_day", usable_day).set("coupon_rule", coupon_rule)
				.set("consume_money", consume_money).set("is_seller", user_id)
				.set("is_course", title)
				.set("category_01_id", category_01_name)
				.set("category_02_id", category_02_name)
				.set("post_time", DateUtils.getCurrentDateTime())
				.set("coupon_number", DateUtils.getTimeStampNo())
				.update();
		if (update) {
			redirect("/admin/CouponManage/list?message1=3");
		} else {
			redirect("/admin/CouponManage/list?message1=4");
		}
	}
	/**
	 * 优惠劵记录列表
	...
	public void recordList() {
		...
		if (!kw.equals("")) {
			kw = kw.trim();
			my_sql = my_sql + "  and (provider like '%" + kw + "%' or coupon_name like '%" + kw + "%' or coupon_number like '%" + kw + "%') ";
			message = "search";
		}
		if (!start_time.equals("") && !end_time.equals("")) {
			my_sql = my_sql + " and ( post_time between '" + start_time + "' and '" + end_time + "') ";
		...
		setAttr("kw", kw);
		setAttr("action", message);
		Page<MyCoupon> couponPage = MyCoupon.dao.paginate(CurrentPage, PAGE_SIZE,
				"select * ", "from my_coupon where status="+status+" and " + my_sql + "order by coupon_number desc, post_time desc");
		for (MyCoupon coupons : couponPage.getList()) {
			int user_id = coupons.get(coupons.user_id);
		...
		MyCoupon pass_count = MyCoupon.dao
				.findFirst("select count(*) as pass_count from my_coupon where status=-1 and "+my_sql);
		MyCoupon normol_count = MyCoupon.dao
				.findFirst("select count(*) as normol_count from my_coupon where status=1 and "+my_sql);
		MyCoupon done_count = MyCoupon.dao
				.findFirst("select count(*) as done_count from my_coupon where status=2 and "+my_sql);
		
		setAttr("pass_count", pass_count.get("pass_count"));
		setAttr("normol_count", normol_count.get("normol_count"));
		setAttr("done_count", done_count.get("done_count"));
		
		setAttr("list", couponPage);
		setAttr("pn", CurrentPage);
		render("/admin/RecordCouponManageList.html");
	}
}
